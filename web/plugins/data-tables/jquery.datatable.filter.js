...
(function ($) {
    ...
    function fnCreateDateInput() {

        var id = sTableId + 'date_' + i;
        var input = $('<input type="text" class="search_init text_filter" id="' + id + '" placeholder="' + label + '" />');
        th.html(input);
        input.datepicker();
        th.wrapInner('<span class="filter_column filter_text" />');
        var index = i;
        ...
        $('#' + id).change(function () {
            var dInputDate = input.datepicker("getDate");
            if(dInputDate != null || this.value.trim() == '')
                dInputDate != null && this.value.trim() != '' ? oTable.fnFilter('^'+this.value+'$', index, true) : oTable.fnFilter('', index, true);
            else
                oTable.fnFilter('^$', index, true);

            if(input.val().trim() != '') {
                if (input.hasClass("search_init")) {
                    input.removeClass("search_init");
                }
            }
            else {
                if (!input.hasClass("search_init")) {
